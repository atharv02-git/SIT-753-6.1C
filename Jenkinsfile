pipeline {
    ...
    stages {
        ...
        stage('Security Scan') {
            steps {
                echo 'Security scan help identify vulnerabilities, weaknesses and other security risks in codebases.'
                echo 'Veracode: '
    ...
    post {
        always {
            ...
            emailext (
                to: env.NOTIFY_EMAIL,
                subject: 'Pipeline Build Log',
                body: 'Pipeline execution completed. Please find attached build log.',
                attachLog: true // Attach the build log
            )
        }
    }
